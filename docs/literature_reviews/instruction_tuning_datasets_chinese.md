...
**2. `allenai/WildChat-1M` — 837,989 conversations, of which **14.24% (119,302) are Chinese** · ODC-BY · Tier: Excellent / 优秀.**
I counted the full `language` column over **all 14 shards = 837,989 rows**: English 478,498 (57.10%), **Chinese 119,302 (14.24%)**, Russian 87,127 (10.40%), French 26,977 (3.22%), Spanish 20,447 (2.44%). **119,302 genuine Chinese conversations** makes WildChat one of the largest sources of *real Chinese user prompts* in existence, and the only one in this review where the prompt distribution was not invented by a model or a translator. I then pulled real Chinese rows out of a deep shard (shard 8/14, row group 6/60) to check quality: they carry `country`, `state`, `language`, `turn`, `toxic`, `redacted` and full `openai_moderation` / `detoxify_moderation` fields, so you can filter precisely. | 我对**全部 14 个分片、共 837,989 行**的 `language` 列做了全量统计：英语 478,498（57.10%）、**汉语 119,302（14.24%）**、俄语 87,127（10.40%）、法语 26,977、西班牙语 20,447。**11.9 万条真实中文对话**——是现存最大的**真实中文用户提问**来源之一，也是本综述中唯一提示分布不由模型或译者虚构的数据；随后从深层分片（第 8/14 片、第 6/60 个 row group）取真实中文行验证质量，每行带国家/省份/语言/轮次/毒性/脱敏标记与完整审核元数据，可精确过滤。
> **⚠️ Methodological warning — shard-position bias.** An earlier partial scan of mine (first 3 shards only) returned 23.27% and would have implied ~195K Chinese conversations. That was wrong by 63%: the per-shard Chinese share **declines monotonically** because the shards are time-ordered and WildChat's user base internationalized away from Chinese over the collection window — shard 0 **25.53%**, shard 3 19.34%, shard 5 13.22%, shard 9 11.15%, shard 11 **2.65%**, shard 13 5.20%. **Never estimate a language share in this dataset from leading shards.** | **⚠️ 方法论警告——分片位置偏差。** 我早先仅扫描前 3 个分片得到 23.27%，据此外推约 19.5 万条，偏高 63%。分片按时间排序，中文占比**单调下降**：第 0 片 25.53%、第 5 片 13.22%、第 11 片 2.65%。**切勿以靠前的分片估计该数据集的语言占比。**
...
**3. `Magpie-Align/Magpie-Qwen2-Pro-200K-Chinese` — 200,000 rows (2 shards × 100,000) · no repo license tag; **project README declares data CC-BY-NC-4.0 (non-commercial)** · Tier: Excellent / 优秀.**
...
> **Deep-offset:** No degradation across both shards. Minor defect: occasional garbled characters in synthesized prompts — one row reads "平衡因子擯至2" (擯 is nonsense there). **Relationship to entry #6:** this set is the *filtered distillate* of a Magpie pool — the card's recipe is input quality at least average, instruct_reward at least -10, language equal to Chinese, dedup, then the 200K longest responses. Take this one for quality off the shelf; take #6 for volume if you will filter yourself. | 两个分片均无退化。**与第 6 条的关系**：本集是对 Magpie 原始池按"质量不低于 average、奖励不低于 -10、语言为中文、去重、取最长 20 万条回复"筛选后的产物；要开箱即用的质量选本条，要规模并自行过滤选第 6 条。小瑕疵：合成提问偶有乱字，如"平衡因子擯至2"（"擯"在此处不成词）。
...
**5. `lhoestq/Infinity-Instruct-Chinese-Only` — 751,313 rows, 100% Chinese · ungated mirror of a gated original · Tier: Excellent / 优秀.**
**This entry corrects a significant gap.** `BAAI/Infinity-Instruct` itself is gated and I could not read a row of it — but the Chinese slice of its `7M` config has been extracted and republished **ungated** at this repo, and I verified it: **751,313 rows, every row `langdetect: zh-cn`**, 12 shards. I sampled offset 0 and deep-probed the **final shard (12/12, row groups 4 and 8 of 9)**. Rows are natively Chinese, substantive, and include real multi-turn. Of BAAI's 751,313 Chinese rows in `7M`, **746,970 (99.4%) come from the `Subjective` (general chat) source**, which matches the `source: Subjective` I saw on every sampled row. | **本条修正了一处重要缺口。** `BAAI/Infinity-Instruct` 本身受限、我无法读取任何一行，但其 `7M` 配置的中文切片已被抽出并以**无门禁**形式重新发布于此库，我已验证：**751,313 行，每行 `langdetect: zh-cn`**，12 个分片；采样 offset 0 并深探**末尾分片**（12/12，第 9 个 row group 中的第 4、8 组）。行文原生中文、内容扎实，且含真实多轮。
> 因果联系原则是法律责任归责的一个重要原则，它的含义是( ) A. …D. → 以刑事责任为例分析。如果危害行为与损害结果之间不存在因果关系，当然谈不上构成犯罪…据此，排除选项C。
> (last shard) 云南普洱的温度下降了18℃，现在的温度为6℃，原来的温度是多少？ → 设原来的温度为x℃…x = 24 / 好的，现在请你告诉我云南普洱的年降水量是多少。 → 约在1200毫米左右…（真多轮 / genuine multi-turn）
> **Deep-offset:** No degradation at the final shard. **Caveat:** the mirror carries **no license tag**; the upstream BAAI repo is CC-BY-SA-4.0 (share-alike), which is what actually governs. Prefer this over `Mxode/Chinese-Instruct`'s `infinity-instruct` config (386,426 rows) — that one keeps **single-turn only**, so this mirror gives you ~2× the rows *and* the multi-turn structure. | 末尾分片无退化。**注意**：镜像库**无许可标签**，实际适用上游 BAAI 的 CC-BY-SA-4.0（传染性共享）。相较 `Mxode/Chinese-Instruct` 的 `infinity-instruct` 子集（386,426 行，仅保留单轮），本镜像行数约为其两倍且保留多轮结构，应优先选用。

**6. `Magpie-Align/Magpie-Qwen2-Air-3M-v0.1` — 3,000,000 rows, **~71–74% Chinese (≈2.13M zh rows)** · Tier: Excellent as a *pool*, not as a drop-in / 优秀（作为待筛选池，而非即用数据）.**
**The largest Chinese instruction resource in this entire review, and the one most likely to be overlooked** — its name says neither "Chinese" nor anything about language. I counted the `language` column over **4 shards = 428,572 rows: ZH 315,811 (73.69%)**, EN 95,845 (22.36%), null 5,142, JA 1,648; a full-repo scan puts it at **2,133,622 zh rows (71.1%)**. Teacher is **`Qwen2-7B-Instruct`** — a 7B model, not the 72B used for the 200K-Chinese release — and, crucially, **this is the raw unfiltered pool**: the curated 200K-Chinese set is what you get *after* filtering it. | **本综述中最大的中文指令资源，也是最容易被漏掉的一个**——其名称既未提"Chinese"也未提任何语言。我对 **4 个分片、428,572 行**的 `language` 列统计：**中文 315,811（73.69%）**、英文 95,845（22.36%）；全库扫描为 **2,133,622 行中文（71.1%）**。教师模型是 **`Qwen2-7B-Instruct`**（7B，而非 200K-Chinese 版所用的 72B），且关键在于**这是未过滤的原始池**——策展后的 200K-Chinese 正是筛选它的产物。
> 后端开发使用什么数据库？ → 后端开发中使用的数据库类型取决于项目需求、性能要求、数据结构、可扩展性、成本等因素…（`instruct_reward: 2.15`, `input_quality: average`）
> **Deep-offset / quality spread:** Stable in *composition* but **wide in quality**, exactly as an unfiltered pool should be. Sampled rows include `instruct_reward: -6.31` ("为了提供一个准确的回答，我需要知道具体的句子和选项是什么" — the model refusing for lack of context) and `-9.11`. Chinese and English rows are **interleaved**, so you must filter on `language == "ZH"` *and* threshold `instruct_reward` / `input_quality`. Do that and it yields far more usable Chinese than any other single source; use it raw and it will hurt you. | 组成稳定但**质量跨度极大**，正符合未过滤池的特征：采样中出现 `instruct_reward` 为 −6.31、−9.11 的行（模型因缺少上下文而拒答）。中英文行**交错排列**，必须同时按 `language == "ZH"` 过滤并对奖励分/输入质量设阈值。做了这一步，它能产出比任何单一来源都多的可用中文；直接原样使用则会有害。

**7. `Mxode/Chinese-Instruct` — 4,845,389 rows across 14 configs · CC-BY-SA-4.0 · Tier: Excellent (aggregate) / 优秀（聚合）.**
...
**8. `BelleGroup/train_3.5M_CN` — 3,606,402 rows · **GPL-3.0 tag but research-only prose** · Tier: Good / 良好.**
...
**9. `YeungNLP/firefly-train-1.1M` — 1,649,399 rows (card says 1.15M) · **no license** · Tier: Good / 良好.**
...
**10. `wangrui6/Zhihu-KOL` — 1,006,218 rows · **no license (scraped Zhihu content)** · Tier: Good, degrading / 良好但递减.**
...
**11. `BAAI/COIG` — 178,246 rows in the `Default` split (card total 190,251; 275,985 across configs) · Apache-2.0 · Tier: Good / 良好.**
...
**12. `BAAI/COIG-PC-Lite` — 1,078,563 rows across splits (`train` split 216,691) · license "unknown", **non-commercial gate on the parent** · Tier: Good (task data) / 良好（任务型）.**
...
**13. `m-a-p/COIG-Kun` — 367,592 rows (`skypile` split 57,339 · `wanjuan` · `wudao`) · **no dataset license** · Tier: Good / 良好.**
...
**14. `TigerResearch/sft_zh` — 530,705 rows · Apache-2.0 · Tier: Good / 良好.**
...
**15. `YeungNLP/moss-003-sft-data` (mirror of `fnlp` / `OpenMOSS-Team`) — 670,948 rows in this mirror (official card: 1,074,551 conversations) · CC-BY-4.0 tag / **CC-BY-NC-4.0 per project README** · Tier: Good but half-English / 良好但半英文.**
...
**16. `Hello-SimpleAI/HC3-Chinese` — 12,853 questions in the `all` config (baike 4,617 · open_qa 3,293 · nlpcc_dbqa 1,709 · psychology 1,099 · medicine 1,074 · finance 689 · law 372) · CC-BY-SA-4.0 · Tier: Good (small, special-purpose) / 良好（小、专用）.**
...
**17. `Azure99/blossom-v6-sft-stage1` — 149,750 rows · Apache-2.0 · Tier: Good but ~half English / 良好但约半英文.**
...
**18. `llm-wizard/alpaca-gpt4-data-zh` — 48,818 rows · card tag CC-BY-4.0 but **upstream is CC-BY-NC-4.0** · Tier: Mixed / 参差.**
...
**19. `FreedomIntelligence/Evol-Instruct-Chinese-GPT4` — 70,000 rows · **no license** · Tier: Mixed / 参差.**
...
**20. `shibing624/sharegpt_gpt4` — 103,415 rows · CC-BY-4.0 tag, GPT-4-output-derived · Tier: Mixed / 参差.**
...
**21. `CohereLabs/aya_collection_language_split` (`chinese` config) — 58,941 train / 7,397 val / 8,634 test · Apache-2.0 · Tier: Mixed / 参差.**
...
**22. `CohereLabs/aya_dataset` — 202,362 train rows total, of which **Simplified Chinese 3,038 (1.50%) and Traditional Chinese 1,871 (0.92%)** · Apache-2.0 · Tier: Mixed (tiny zh share) / 参差（中文占比极小）.**
...
**23. `Chinese-Vicuna/instruct_chat_50k.jsonl` — 51,584 rows · Apache-2.0 · Tier: Mixed / 参差.**
...
**24. `silk-road/Wizard-LM-Chinese-instruct-evol` — 70,000 rows · CC-BY-4.0 · Tier: Poor / 较差.**
...
**25. `dikw/hh_rlhf_cn` — 344,317 train rows · license tag `llama2` · Tier: Poor / 较差.**
...
**26. `beyond/rlhf-reward-single-round-trans_chinese` — 19,862 train rows · **no license** · Tier: Poor / 较差.**
...
**27. `BelleGroup/multiturn_chat_0.8M` — 831,036 rows · GPL-3.0 tag / research-only prose · Tier: Poor *as shipped* / 原样不宜用.**
...
**28. `BelleGroup/school_math_0.25M` — 248,481 rows · GPL-3.0 tag / research-only prose · Tier: Poor (unverified solutions) / 较差（解答未校验）.**
...
**29. English-dominant — sampled, measured, and excluded as zh sources / 以英文为主：已采样实测，不作为中文来源.**
...
- `allenai/tulu-3-sft-mixture` — 939,343 rows, ODC-BY. **Full-repo CJK scan: 11,332 rows (1.21%) Chinese**, concentrated in `tulu_v3.9_wildchat_100k` (5,647) and `tulu_v3.9_aya_100k` (5,018); seven subsets have zero. So Tulu 3's Chinese is **WildChat's and Aya's Chinese, subsampled** — go to those directly. Excellent English/multilingual mixture; at ~1% zh it is an English anchor, not a zh resource. | 全库中日韩字符密度扫描：**11,332 行（1.21%）为中文**，集中于 `tulu_v3.9_wildchat_100k`（5,647）与 `tulu_v3.9_aya_100k`（5,018），七个子集为零。即 Tulu 3 的中文就是**被下采样的 WildChat 与 Aya 中文**，不如直接取用其源头。作为英文锚点优秀，但约 1% 的中文占比说明它不是中文数据来源。
- `Magpie-Align/Magpie-Qwen2.5-Pro-1M-v0.1` — 1,000,000 rows, 16 shards. I counted the full `language` column over **2 of 16 shards = 125,000 rows**: **EN 124,935 (99.95%)**, Latin 29, Xhosa 12, Indonesian 6, German 5 — **zero Chinese in those two shards**. ⚠️ **Correction from a full-repo scan: the dataset overall is 35,914 Chinese rows (3.6%)** — the Chinese is concentrated in shards my 2-shard sample did not touch, the same shard-position bias that inflated my first WildChat estimate. Either way it is ~96% non-Chinese and the `pre_query_template` is the plain English Qwen system prompt, i.e. it was **not** steered toward Chinese the way the 200K-Chinese release was. Use `Magpie-Qwen2-Pro-200K-Chinese` instead. | 对 **16 个分片中的 2 个、共 125,000 行**的 `language` 列全量统计：**英语 124,935（99.95%）**，**该两分片中中文为零**。⚠️ **据全库扫描修正：全库共有 35,914 行中文（3.6%）**——中文集中在我未覆盖的分片，与早先高估 WildChat 的是同一种分片位置偏差。无论如何该集约 96% 非中文，`pre_query_template` 为英文 Qwen 系统提示，并未像 200K-Chinese 版那样引导为中文。
...
|---|---|---|
| `BAAI/Infinity-Instruct` | `gated: auto`; Parquet branch returned non-Parquet bytes (auth wall) — **could not read a single row of the BAAI repo** | CC-BY-SA-4.0; `7M` = 7,449,106 rows, of which **751,313 (10.09%) Chinese**. ✅ **Fully worked around:** `lhoestq/Infinity-Instruct-Chinese-Only` (**751,313 rows, 100% zh, ungated**) and `manifoldlabs/Infinity-Instruct` (all configs, ungated) carry the same content — I sampled the former at offset 0 and at the final shard. / BAAI 原库受限，但已完全绕过：`lhoestq/Infinity-Instruct-Chinese-Only`（751,313 行、100% 中文、无门禁）与 `manifoldlabs/Infinity-Instruct`（全配置、无门禁），前者已实测。 |
| `lmsys/lmsys-chat-1m` | `gated: auto`; Parquet read failed with an auth wall | Exactly **1,000,000** conversations across 25 models, **Chinese 24,499 (2.45%)** — recovered from a byte-identical mirror, cite as mirror-derived. ⚠️ **No `license:` tag**; access is under the bespoke "LMSYS-Chat-1M Dataset License Agreement". / 恰 100 万条，中文 24,499（2.45%），数字来自逐字节一致的镜像；**无许可标签**，须接受专有协议。 |
...
| `m-a-p/neo_sft_phase2` | HF API returns **401/404 — the repo has been withdrawn** (a Wayback snapshot confirms it was public: **109,447 rows**, apache-2.0, ungated) | From MAP-Neo (arXiv:2403.19327): **>100k multi-turn dialogues from real user conversations** + 5k math/code — not self-instruct, not distilled from a named teacher. **52.6% Chinese (57,621 rows)**. Survives via mirrors `REILX/neo_sft_phase2_multi` (109,447), `REILX/neo_sft_phase2_single` (93,953), `svjack/neo_sft_phase2_zh` (52,576 zh), and as `Mxode/Chinese-Instruct` config `neo_sft_phase2` (58,549 zh). ⚠️ `m-a-p/neo_sft_phase1` returns 401 with **no Wayback snapshot — treat as unverified**. / 原库已撤下（存档显示曾公开，109,447 行）；含逾 10 万条真实用户多轮对话，**52.6% 为中文**；经上述镜像留存。 |
...
**The headline finding of this review: for Chinese, the constraint is not volume, it is *provenance*.** There are well over 10M open zh instruction rows, but the overwhelming majority are one of three things — machine-translated English data, `gpt-3.5-turbo` self-instruct output from 2023, or re-packagings of the previous two. Only five sources in this entire review supply Chinese at scale that a Chinese person actually wrote or that a strong model generated *natively in Chinese*: **WildChat's Chinese slice** (119,302 real user conversations), **COIG-CQIA** (human answers, human-verified), **Magpie-Qwen2-Pro-200K-Chinese** (natively self-synthesized by Qwen2-72B), **Infinity-Instruct's Chinese slice** (751,313 rows, reachable only through an ungated mirror), and **Chinese-DeepSeek-R1-Distill-110k** (R1 reasoning on Chinese prompts). Everything else should be treated as filler or as breadth, not as the thing that teaches your model how Chinese assistants talk.
/ **本综述的核心结论：中文的瓶颈不是数据量，而是数据来源。** 公开中文指令数据远超一千万条，但绝大多数不外乎三类——英文数据的机器翻译、2023 年 `gpt-3.5-turbo` 的 self-instruct 产物，或前两者的再打包。整篇综述中只有五个来源在规模上提供了**中国人真正写的**或**强模型以中文原生生成的**中文：**WildChat 的中文切片**（119,302 条真实用户对话）、**COIG-CQIA**（人写答案且经人工校验）、**Magpie-Qwen2-Pro-200K-Chinese**（Qwen2-72B 原生自合成）、**Infinity-Instruct 的中文切片**（751,313 行，仅可经无门禁镜像获取）、**Chinese-DeepSeek-R1-Distill-110k**（R1 在中文提示上的推理）。其余应视为填充或广度补充，而非教会模型"中文助手怎么说话"的主体。
...
|---|---|---|---|---|
| 1 | `Magpie-Align/Magpie-Qwen2-Pro-200K-Chinese`, filtered `instruct_reward > 0` and `input_quality ∈ {good, excellent}`, deduped on `min_neighbor_distance` | ~120K | Natively-Chinese general chat backbone, **generated by a Qwen-family teacher** — the closest possible match to a Qwen-class student's own distribution. Ships its own reward/quality/dedup metadata. | ⚠️ No repo tag, but **project README says CC-BY-NC-4.0**; Qwen2-72B outputs (Qwen license) |
| 2 | `allenai/WildChat-1M`, `language == "Chinese"`, `toxic == false`, `redacted == false`, length- and turn-filtered | **~100K of the 119,302 available** | **Real Chinese user prompts.** Nothing else in the open ecosystem fixes the prompt-distribution gap. Multi-turn comes free (`turn` field). | ⚠️ ODC-BY, but responses are OpenAI outputs |
| 2b | `lhoestq/Infinity-Instruct-Chinese-Only`, subsampled | ~150K of **751,313** | **The correction that most changes this recipe.** BAAI/Infinity-Instruct is gated, but its full 751K-row Chinese slice is republished ungated and I deep-verified it — natively Chinese, substantive, genuinely multi-turn, ~2× the rows of the Mxode route *and* it keeps the multi-turn structure. | ⚠️ Mirror has no tag; upstream is **CC-BY-SA-4.0 (share-alike)** |
...
| 5 | `Mxode/Chinese-Instruct`, configs `stem_zh` + `firefly` + `neo_sft_phase2`, subsampled | ~150K | Breadth: STEM explanation and 23 Chinese NLP task types, with a declared license the originals lack. **Drop its `infinity-instruct` config** now that component 2b supplies the same source at 2× the rows with multi-turn intact, and **skip the `magpie` config** (translationese, sister/elder-sister slip at depth). | ✅ CC-BY-SA-4.0 (share-alike is copyleft — check compatibility with your release plan) |
| 5b | *Optional volume:* `Magpie-Align/Magpie-Qwen2-Air-3M-v0.1`, `language == "ZH"` **and** `instruct_reward` / `input_quality` thresholded | up to **2.13M zh available** | The largest Chinese pool in this review by an order of magnitude. Only reach for it if components 1-5 leave you short — the teacher is Qwen2-**7B**, so quality per row is below component 1, and it is **unfiltered** (sampled rows at reward -6.3 and -9.1). | ⚠️ Same Magpie CC-BY-NC-4.0 / Qwen terms |
...
**Preference / DPO stage** / **偏好与 DPO 阶段**: the open Chinese preference landscape is genuinely weak. `BAAI/Infinity-Preference` (59,338) and `Skywork/Skywork-Reward-Preference-80K-v0.2` (77,016) are **English** in every row I sampled; `dikw/hh_rlhf_cn` and `beyond/rlhf-...-trans_chinese` are broken machine translations. The usable options are **`COIG-P`** (1,009k zh preference pairs across Chat/Code/Math/Logic/Novel/Role, built with 15 LLMs, arXiv:2504.05535), **Huozi 活字's `huozi_rlhf_data.csv`** (**16,918 pairs, human-annotated** over Huozi's own model outputs — one of the very few genuinely human-labelled Chinese preference sets, and **GitHub-only**, since the HIT-SCIR HF org has no datasets), **CValues-Comparison** (145,669 pairs, safety/responsibility), and **`Hello-SimpleAI/HC3-Chinese`** repurposed as human-vs-ChatGPT preference pairs (12,853 questions). If neither fits, generating on-policy preference pairs over your own SFT prompts is likely to beat anything currently public in Chinese. | 开源中文偏好数据确实薄弱：`Infinity-Preference` 与 `Skywork-Reward-Preference-80K-v0.2` 在我采样的每一行都是英文；两个 HH 中译版本质量破损。可用者为 **COIG-P**（约 100.9 万条中文偏好对）、**活字 Huozi 的 `huozi_rlhf_data.csv`**（**16,918 条人工标注**，是极少数真正人工标注的中文偏好集，且**仅存于 GitHub**）、**CValues-Comparison**（145,669 对，安全与责任）以及把 **HC3-Chinese** 改用为"人类 vs ChatGPT"偏好对。若都不合适，在自有 SFT 提示上做 on-policy 偏好采样，很可能优于目前任何公开中文偏好集。

**Totals and overlaps** / **总量与重叠**: the components above sum to roughly **770K examples** (120K + 100K + 150K + 70K + 80K + 150K + 20K + 80K), of which ~90% is Chinese. Three overlaps must be deduplicated before training: (a) component 7's Chinese rows come from the same WildChat pool as component 2 — Tulu 3's zh is literally a 100K WildChat subsample; (b) component 5's dropped `infinity-instruct` config is a single-turn subset of component 2b; (c) `Mxode`'s `coig-cqia` config (10,306 rows) overlaps component 4. Deduplicate on normalized instruction text. | 上述各项合计约 **77 万条**，其中约 90% 为中文。训练前须去重三处重叠：(a) 成分 7 的中文行与成分 2 同源于 WildChat；(b) 成分 5 中被舍弃的 `infinity-instruct` 子集是成分 2b 的单轮子集；(c) `Mxode` 的 `coig-cqia` 子集与成分 4 重叠。建议按归一化指令文本去重。
...
- **(B) Real user prompts, model answers / 真实用户提问＋模型作答:** WildChat-1M (**zh 14.24% = 119,302**) and WildChat-4.8M (**zh 266,258**), LMSYS-Chat-1M (gated, zh 24,499), MOSS-003 (built on ~100K beta-user inputs), neo_sft_phase2 (withdrawn; zh 57,621), ShareGPT-derived sets.
...
- **(D) Prompt-free self-synthesis / 无提示自合成:** Magpie family — **Qwen2-Air-3M (zh 2,133,622, Qwen2-7B teacher, unfiltered pool)**, Qwen2-Pro-200K-Chinese (zh 200,000, Qwen2-72B, curated), Qwen2-Pro-1M (zh 290,551), Qwen2.5 line (largely English).
...
- **(H) Curated mega-mixtures / 策展巨型混合:** Infinity-Instruct (7.4M, gated — **but its 751,313-row Chinese slice is mirrored ungated**), Mxode/Chinese-Instruct (4.85M), deepctrl-sft-data (~12M, gated), Tulu 3 (English-centric, zh 1.21%).
...
- **(J) Preference / DPO / RLHF (zh) / 中文偏好数据:** COIG-P (1,009k), **Huozi 活字 (16,918, human-annotated, GitHub-only)**, CValues-Comparison (145,669), Infinity-Preference (8.4% zh), Skywork-Reward-Preference (0.03% zh), hh_rlhf_cn (MT), Chinese-dpo-pairs, zhihu_rlhf_3k (3,460).
...
- **Size / 规模:** **837,989 conversations** (`datasets-server` `/size`), 14 Parquet shards. **Measured over all 14 shards: English 478,498 (57.10%), Chinese 119,302 (14.24%), Russian 87,127 (10.40%), French 26,977 (3.22%), Spanish 20,447 (2.44%).** ⚠️ The Chinese share **declines monotonically by shard** (0: 25.53% · 3: 19.34% · 5: 13.22% · 9: 11.15% · 11: 2.65% · 13: 5.20%) because shards are time-ordered — a partial scan of leading shards overstates it by more than 60%.
- **Related releases / 相关版本:** `allenai/WildChat` (650K release) **529,428 rows, 101,816 Chinese (19.23%)**; `allenai/WildChat-4.8M` **3,199,860 rows, 266,258 Chinese (8.32%)** — the largest single pool of real Chinese user conversations anywhere. The public repos ship fewer rows than their names suggest because toxic and PII-flagged conversations were stripped; only the `-Full` toxic-inclusive variants (`WildChat-1M-Full` 990,372, `WildChat-4.8M-Full` 4,743,336) remain **gated (manual approval)**.
...
- **License / 许可:** **ODC-BY** since 2024-06-26, applied **retroactively** — the paper's "AI2 ImpACT" statement is out of date, and the ImpACT-era gate was lifted at the same time, so **`WildChat-1M`, `WildChat` and `WildChat-4.8M` are NOT gated** (a common misconception). ⚠️ Responses are OpenAI outputs → OpenAI ToU competing-model restrictions apply independently of the ODC-BY tag.
...
- **Size / 规模:** **exactly 1,000,000 conversations**, 25 models, 210,479 users, 154 languages, avg 2.0 turns. **Chinese = 24,499 (2.45%)**; top languages English 777,453 (77.75%), Portuguese 28,616, unknown 27,922, Russian 27,596, then Chinese. ⚠️ The paper gives this only as a bar chart with no numeric table; the count was recovered from a **byte-identical unofficial mirror** (parquet bytes exactly matching the official `download_size`), so **cite it as mirror-derived**.
...
- **License / 许可:** ⚠️ **No `license:` tag on the repo at all.** Access is conditioned on the bespoke **"LMSYS-Chat-1M Dataset License Agreement"** — not an OSI, CC or ODC licence. Outputs mix OpenAI-ToS and Llama-licensed models depending on subset.
...
### OpenHermes-2.5 and its Chinese translations / OpenHermes-2.5 及其中文翻译版
- **Venue / Link:** **No paper** — `teknium/OpenHermes-2.5` ships only a self-published `@misc` citation. https://huggingface.co/datasets/teknium/OpenHermes-2.5
- **Size / 规模:** **1,001,551 examples.** Verified per-source counts: glaive-code-assist 182,240 · CamelAI 78,390 · metamath 56,448 · EvolInstruct_70k 51,948 · cot_alpaca_gpt4 42,026 · airoboros2.2 35,380 · platypus 22,280 · GPT-4 Comparison 14,928 · UnnaturalInstructions 8,610 · CogStackMed 4,443, plus SlimOrca ~550K, GPT-4-only ShareGPT, GPT4-LLM and GPTeacher.
- **Construction / 构建方式:** a compilation of ~16 existing, mostly GPT-4-synthesized datasets — the repo's own tags are literally `synthetic`, `GPT-4`, `Distillation`.
- **License / 许可:** ⚠️ **None declared** — no tag, nothing in the card YAML — on top of heavy GPT-4 derivation and unlicensed upstreams (WizardLM Evol-Instruct). **Do not describe it as openly licensed.**
- **Chinese / 中文含量:** **effectively zero — 1,190 / 1,001,551 rows (0.119%) are Chinese-dominant**; only 0.45% contain any CJK at all. The `language` column is 99.84% null.
- **Chinese translations — exactly two exist, neither citable as a standard resource / 中文版仅两个，均不足以作为标准资源:**
  - `ldwang/OpenHermes-2.5-zh` — **1,001,551 rows**, a complete 1:1 translation keeping both `conversations` (EN) and `zh_conversations` (ZH), so it doubles as a parallel corpus. ⚠️ **The translator model is stated nowhere — unverified.** Declared Apache-2.0, which is unsupportable given the source declares no licence. ⚠️ **Quality defect found on inspection: in the first row the `zh_conversations` *human* turn contains a translated answer rather than the question — role alignment is broken in at least that sample.** ~1 like, ~63 monthly downloads.
  - `wenbopan/OpenHermes-2.5-zh` — **91,506 rows**; the card states only ~10% of the original was translated, **using GPT-3.5**, with low-quality translations filtered. Same Apache-2.0-over-nothing problem. ~10 likes, ~53 monthly downloads.
- **Bottom line / 结论:** **there is no notable, citable Chinese OpenHermes.** Both are single-uploader artifacts with no paper, negligible usage, and a licence assertion their source cannot support. | **不存在值得引用的中文版 OpenHermes**：两者均为个人上传、无论文、使用量极低，且其许可声明是源数据无法支撑的。
...
- **Chinese content across the whole Magpie family / Magpie 全家族的中文含量** (all ungated; zh counts from full `language`-column scans, with my own partial-scan confirmations noted):

  | HF dataset | teacher | rows | **zh rows** | zh % |
  |---|---|---|---|---|
  | **`Magpie-Qwen2-Air-3M-v0.1`** | Qwen2-**7B**-Instruct | 3,000,000 | **2,133,622** | **71.1%** (my 4-shard scan: 315,811 / 428,572 = **73.69%**) |
  | `Magpie-Qwen2-Pro-1M-v0.1` | Qwen2-72B-Instruct | 1,000,000 | **290,551** | 29.1% |
  | `Magpie-Qwen-Air-300K-Filtered` | Qwen2-7B-Instruct | 300,000 | **103,230** | 34.4% |
  | **`Magpie-Qwen2-Pro-200K-Chinese`** | Qwen2-72B-Instruct | 200,000 | **200,000** | 100% |
  | `Magpie-Qwen2-Pro-300K-Filtered` | Qwen2-72B-Instruct | 300,000 | **39,739** | 13.2% |
  | `Magpie-Qwen2.5-Pro-1M-v0.1` | Qwen2.5-72B-Instruct | 1,000,000 | **35,914** | 3.6% |
  | `Magpie-Qwen2.5-Pro-300K-Filtered` | Qwen2.5-72B-Instruct | 300,000 | **73** | 0.02% |
  | `Magpie-Qwen2-Pro-200K-English` | Qwen2-72B-Instruct | 200,000 | 0 | 0% |

  **The headline correction: the canonical "Chinese Magpie" is the 200K set, but the largest Chinese Magpie resource by far is `Magpie-Qwen2-Air-3M-v0.1` with ~2.13M Chinese rows — roughly 10× the 200K release.** The trade-off is teacher size (7B vs 72B) and filtering (none vs curated). The Qwen2.5 line went the other way: its 300K-Filtered recipe explicitly favours English (quality at least good, reward at least -5, drop instructions mentioning "Alibaba"), which is why Chinese collapses to 73 rows. Across all 44 Magpie-Align datasets, no other explicitly Chinese or multilingual release exists — the rest are Llama-3/3.1/3.3, Phi-3, Gemma-2, reasoning, math/coder and DPO variants, all English in practice. | **核心修正：公认的"中文 Magpie"是 200K 版，但真正最大的中文 Magpie 资源是 `Magpie-Qwen2-Air-3M-v0.1`，约 213 万行中文，约为 200K 版的十倍**；代价是教师模型更小（7B 对 72B）且未经过滤。Qwen2.5 线则相反，其 300K-Filtered 配方明显偏向英文，故中文仅剩 73 行。
...
- **License / 许可:** ⚠️ **No `license:` tag on ANY of the Qwen-generated repos** (verified across Qwen2-Pro-1M, Qwen2.5-Pro-1M, Qwen2-Pro-200K-Chinese, Qwen2.5-Pro-300K-Filtered, Qwen2-Air-3M). But the **Magpie GitHub README declares code MIT and DATA CC-BY-NC-4.0 — non-commercial**. Llama-generated Magpie repos are tagged `llama3`/`llama3.1`. So for every Chinese-relevant set: **non-commercial per the project, no tag on the repo, and Qwen model terms on the outputs.**
...
- **Size / 规模 (measured, not card claims):** **Aya Dataset = 204,112 rows (train 202,362 + test 1,750)**, of which **Simplified Chinese 3,038 (1.50%)** and **Traditional Chinese 1,871 (0.92%)** in train — I counted the full `language` column, not a sample — plus 250 `zho` test rows, for **5,159 Chinese rows in total**. The corpus is dominated by Plateau Malagasy 14,597 (7.21%), Sinhala 14,524 (7.18%), Tamil 14,133 (6.98%), Yoruba 11,758 (5.81%). **Aya Collection: 513,757,344 rows over 114 languages.** Its Chinese configs are far larger than the small `chinese` one I sampled: **`simplified_chinese` 4,177,708 · `traditional_chinese` 4,122,013 · `cantonese` 4,120,142 · `chinese` 74,972** (train 58,941 / val 7,397 / test 8,634). The small `chinese` config is only UNER-LLM-inst NER (7,992) + Xlel_wd event linking (50,949); the multi-million configs are dominated by **machine-translated templated data** (SODA-inst 1,191,582 · Wiki-split-inst 989,944 · Xlel_wd 523,112 · HotpotQA 355,476 · NQ-Open 175,850 · CNN-DailyMail 100,000), i.e. **translationese, not native Chinese**.
- **⚠️ Naming trap / 命名陷阱:** the `achinese` config (8,242,284 rows) is **Acehnese (Indonesia), not Chinese.** It is easy to grep for "chinese" and pull in 8.2M rows of the wrong language. | `achinese` 配置（8,242,284 行）是**印尼亚齐语，不是中文**；按 "chinese" 字符串匹配会误引入 824 万行错误语种。
...
- **Size / 规模:** **110,000 rows ≈ 150.6M tokens** (4.3M prompt + 54.7M answer + 91.6M reasoning, summed from the dataset's own token-length columns) — Math 36,568 · Exam 2,432 · STEM 12,648 · General 58,352 (弱智吧、逻辑推理、小红书、知乎、Chat). **105,740 / 110,000 (96.1%) have a Chinese prompt.** Measured `repo_name` mix: coig/neo 52,893 · EduChat-Math 19,729 · GSM8K_zh 8,776 · applied_math 7,493 · stem_zh 12,648 · zhihu 2,534 · xhs 1,507 · **ruozhiba 240**. Fields: `input`, `reasoning_content`, `content`, `repo_name`, `prompt_tokens_len`, `content_tokens_len`, `reasoning_content_tokens_len`, `score`.
...
- **License / 许可:** ✅ **Apache-2.0.** DeepSeek-R1 is MIT and explicitly permits distillation — the cleanest teacher provenance of any large set in this review. Scores come from Qwen2.5-72B-Instruct (Qwen licence).
- **⚠️ Nothing is filtered out.** The `score` column spans the full range and **2,703 rows are scored 0**; the release ships them anyway. Thresholding is the user's job — which is why the recommendation above specifies `score >= 9`. | **⚠️ 该集未做任何剔除**：`score` 覆盖全域，其中 **2,703 行得分为 0** 仍随包发布，过滤须由使用者自行完成。
...
- **Size and measured Chinese share / 规模与实测中文占比:** **InfInstruct-F-7.4M = 7,449,106** foundational instructions curated from over 100M candidates; **InfInstruct-G-1.5M = 1,456,927** chat instructions; `3M` = 3,463,473; `0625` = 659,808. The paper publishes no language breakdown, but full scans of the `langdetect` column give:

  | config | rows | en | **zh** | zh % |
  |---|---|---|---|---|
  | `7M` | 7,449,106 | 6,697,793 | **751,313** | **10.09%** |
  | `3M` | 3,463,473 | 2,714,827 | **748,646** | **21.62%** |
  | `Gen` | 1,456,927 | 1,150,778 | **306,149** | **21.01%** |
  | `0625` | 659,808 | 641,610 | **7,972** | **1.21%** |

  Of the 751,313 Chinese rows in `7M`, **746,970 (99.4%) come from the `Subjective` (general-chat) source** — consistent with `source: Subjective` on every row I sampled from the mirror.
...
- **✅ Ungated mirrors — the practical route / 无门禁镜像（实际可用路径）:** the gate is not a dead end. Two mirrors carry the same content ungated and I read rows from both routes:
  - **`lhoestq/Infinity-Instruct-Chinese-Only`** — https://huggingface.co/datasets/lhoestq/Infinity-Instruct-Chinese-Only — **751,313 rows, verified via `/size`, every row `langdetect: zh-cn`**, 12 shards. Exactly the Chinese slice of `7M`. **This is the single most convenient citation for Infinity-Instruct's Chinese portion.**
  - **`manifoldlabs/Infinity-Instruct`** — https://huggingface.co/datasets/manifoldlabs/Infinity-Instruct — ungated, all configs, row counts verified identical (`7M` 7,449,106 · `3M` 3,463,473 · `0625` 659,808).
- **Quality / 质量:** The BAAI repo itself: **not sampled — gated.** The Chinese-Only mirror: **sampled at offset 0 and deep-probed at the final shard (12/12, row groups 4 and 8 of 9)** — natively Chinese, substantive, and genuinely multi-turn (a temperature word-problem followed by a follow-up about Pu'er's annual rainfall). No degradation at depth. Prefer this over `Mxode/Chinese-Instruct`'s `infinity-instruct` config (386,426 rows), which keeps **single-turn only** — the mirror gives ~2× the rows *and* preserves multi-turn structure. | BAAI 原库受限未采样；中文镜像已采样 offset 0 并深探末尾分片，原生中文、内容扎实、含真实多轮，深层无退化。优先于 `Mxode/Chinese-Instruct` 的 `infinity-instruct` 子集（386,426 行，仅单轮）——镜像行数约两倍且保留多轮。
...
- **License / 许可:** ✅ **ODC-BY.**
- **Measured Chinese share / 实测中文占比:** **11,332 / 939,343 rows = 1.21%** on a full-repo CJK scan, concentrated in just two components — `ai2-adapt-dev/tulu_v3.9_wildchat_100k` (5,647) and `ai2-adapt-dev/tulu_v3.9_aya_100k` (5,018); seven subsets contain **literally zero** Chinese. (My own partial scan of 2 of 6 shards gave 2.15%; the full scan supersedes it — the same shard-position bias seen in WildChat.) **Tulu 3's Chinese is essentially WildChat's and Aya's Chinese, subsampled.**
- **⚠️ Teacher models / 教师模型:** the persona subsets were generated with **GPT-4o and Claude 3.5 Sonnet** (the persona cards invoke Anthropic's ToS explicitly) and the WildChat subset with GPT-4 — so **both OpenAI and Anthropic output restrictions sit on top of the ODC-BY tag**. The card also warns that **some portions are non-commercial** (No Robots, CC-BY-NC-4.0, 9,500 rows).
- **Version note / 版本说明:** `allenai/tulu-3-sft-olmo-2-mixture` is the same composition (939,344 rows, same 11,332 zh). Newer `-0225` revisions exist (866,137 / 866,138 rows) but **declare no license tag — unverified**. **There is no "Tulu 3.1" dataset**: `Llama-3.1-Tulu-3.1-8B` is a model-only update (PPO→GRPO in the final RL stage) with unchanged SFT data.
- **Quality / 质量:** Excellent and the most transparent open post-training recipe available, but **English-centric at 1.21% Chinese** — offsets 0 / 2,000 / 20,000 returned English and Spanish (OASST1-converted) rows. Its role in a Chinese SFT mix is as an **English anchor** against the forgetting a zh-only SFT stage induces on a Qwen base, not as a zh resource; and if you want its Chinese, take it from WildChat directly at full resolution. | 全库中日韩字符密度扫描得 **11,332 行中文（1.21%）**，集中于 `tulu_v3.9_wildchat_100k`（5,647）与 `tulu_v3.9_aya_100k`（5,018），七个子集为零——**Tulu 3 的中文本质上就是被下采样的 WildChat 与 Aya 中文**。质量优秀且是最透明的公开后训练配方，但以英文为中心；在中文混合中作**英文锚点**，若要其中文则应直接从其源头全量取用。
...
- **Data / 数据:** https://huggingface.co/datasets/BAAI/Infinity-Preference · **59,438 rows** (train 59,338 + test 100). No dedicated paper.
- **Construction / 构建方式:** instructions sampled evenly across Infinity-Instruct's task-type labels; **preference pairs sampled from Gemma-2-9B-IT** and annotated with task-specific weights plus **ArmoRM** scores. Fully model-generated and model-judged — **no human annotation**.
- **License — ⚠️ CONFLICT:** the tag says `apache-2.0`, but the card's own disclaimer restricts the resources to "**academic research purposes only … cannot be used for commercial purposes**". Gemma-2 outputs additionally invoke the Gemma Terms of Use. **Cite the disclaimer, not the tag.**
- **Quality / 质量:** ⚠️ **Predominantly English.** Every row I sampled at offset 0 is English (PGP/GPG key generation on Ubuntu; propositional-logic reasoning); a CJK scan of the full prompt column finds only **4,971 / 59,338 train prompts (8.4%)** with any meaningful Chinese. Chosen and rejected are near-identical long technical answers — a fine-grained preference signal, but not a Chinese one. | ⚠️ **以英文为主**：offset 0 采样行全为英文；对完整提示列做中日韩字符扫描，仅 **4,971 / 59,338（8.4%）**含实质中文。
...
- **Construction / 构建方式:** Machine translation of Anthropic HH-RLHF into Chinese. String-matching the English parent identifies it precisely as **the HELPFUL splits only** (helpful-base 8,771 + helpful-rejection-sampled 11,324; harmless ≈13) — **not** `Dahoas/rm-static`, which only overlaps 79%. ⚠️ **The translation method is undocumented — no card, no script — so do not attribute it to a named MT system.**
- **Size / 规模:** **24,858 rows** (train 19,862 + test 4,996).
...
### Huozi 活字 RLHF data (HIT-SCIR) — the human-annotated Chinese preference set / 唯一人工标注的中文偏好集
- **Venue / Link:** No dedicated paper. https://github.com/HIT-SCIR/huozi
- **Data / 数据:** `data/huozi-rlhf/huozi_rlhf_data.csv` in the GitHub repo — **16,918 rows** (columns `prompt` / `chosen` / `reject`). ⚠️ **GitHub-only: the `HIT-SCIR` HuggingFace org contains ZERO datasets.**
- **Construction / 构建方式:** the README states verbatim "**16.9k 人工标注的偏好数据，回复来自活字模型**" — **human-annotated preferences over Huozi's own model outputs.** This makes it one of the very few genuinely human-labelled Chinese preference sets in existence.
- **License / 许可:** repo is Apache-2.0, but the README requires **written authorization by email for commercial use** of Huozi models and derivatives; it is ambiguous whether that reaches the CSV. Treat as model-output-derived with a registration requirement.
- **Quality / 质量:** **Not sampled** (GitHub CSV, outside the HF sampling harness used here). Flagged prominently because the review's other Chinese preference options are either machine-translated or English — a 16.9k human-labelled set is small but categorically different. | **未采样**（GitHub CSV，不在本文的 HF 采样流程内）。之所以着重列出，是因为本综述其余中文偏好选项要么是机器翻译、要么实为英文，而这是一份 1.69 万条的人工标注数据，性质上截然不同。

### CValues-Comparison (Alibaba X-PLUG)
- **Venue / Link:** arXiv:2307.09705 (comments field says "Working in Process" — **preprint, no venue**) · https://github.com/X-PLUG/CValues (Apache-2.0)
- **Size / 规模:** **145,669 pairs** (train 116,536 + test 29,133), verified consistent across four HF mirrors. Canonical host is ModelScope `iic/CValues-Comparison` (ModelScope-side license **unverified**).
- **Construction / 构建方式:** self-instruct prompts, **large-model generation with ChatGPT rewriting**, then responses bucketed by a self-trained reward model into 拒绝&正向建议 > 拒绝为主 > 风险回复 and paired across tiers. **Not human-annotated preferences** → OpenAI ToU exposure. 100% Chinese; domains are safety and responsibility across 8 expert areas.
- **HF mirrors (all ungated) / 镜像:** `Skepsun/cvalues_rlhf` (**97,116** = 77,693 + 19,423, apache-2.0) · `mxz/CValues_DPO`, `mxz/CValues`, `xiaodongguaAIGC/CValues{,_DPO}` (116,536 / 29,133 each, **no card, no license**). Companion artifacts: Responsibility-Prompts 0.6k, Responsibility-MC 1.7k, and **100PoisonMpts 0.9k — genuinely human-authored**. The 1.3k safety prompts and 2.6k MC safety prompts are **withheld**.
- **Quality / 质量:** **Not sampled** in this review. | 本综述**未采样**。

### ⚠️ Double-counting trap in the Chinese DPO ecosystem / 中文 DPO 生态的重复计数陷阱
`wenbopan/Chinese-dpo-pairs` (10,735) → `llamafactory/DPO-En-Zh-20k` **zh half (9,999)** → `shibing624/DPO-En-Zh-20k-Preference` **zh half (10,000)** are **the same GPT-3.5-translated pairs** re-packaged three times. Likewise `liyucheng/zhihu_rlhf_3k` == `ticoAg/zhihu_3k_rlhf_{train,test}` (3,260 + 200 = 3,460), and CValues-Comparison has at least five HF mirrors. **Counting these as distinct sources inflates the apparent size of the Chinese preference ecosystem by roughly 3×.** Also note `shareAI/DPO-zh-en-emoji` (2,449 rows): its chosen/rejected pair is **one Chinese and one English answer to the same query**, so the preference signal is *language/style*, not quality. | 上述三者是同一批 GPT-3.5 翻译数据的三次再打包；`zhihu_rlhf_3k` 亦有别名；CValues 至少有五个镜像。**把它们当作不同来源会把中文偏好生态的表观规模夸大约三倍。** 另注意 `shareAI/DPO-zh-en-emoji` 的偏好信号是**语言/风格**而非质量。
...
- **`liyucheng/zhihu_rlhf_3k`** — **3,460 rows** (not 3,000). Preference is **human by proxy: Zhihu upvote counts** (`upvotes_chosen` vs `upvotes_rejected` columns). Declared `cc-by-2.0` by the uploader over scraped user-copyrighted Zhihu content — **legally shaky**. 100% Chinese, ungated, no paper.
...
- **`zake7749/kyara-chinese-preference-rl-dpo-s0-30K`** — **30,152 rows**, prompts drawn from Magpie-Qwen2-Pro-200K-Chinese + stem_zh + Evol-Instruct-Chinese-GPT4. ⚠️ **GATED (`gated: auto`)** and **no license tag**.
- **No Chinese Skywork preference set exists.** The Skywork HF org holds SkyPile-150B (zh pretraining), ChineseDomainModelingEval (zh eval), OR1-RL-Data and the two English Reward sets — nothing else. `Skywork-SynPref-40M` does not appear on HF; release status **unverified**.
...
| 2 | `allenai/WildChat-1M` | real-user logs | **837,989** | real prompts + model answers | gpt-3.5-turbo / gpt-4 | **14.24% = 119,302 (full scan)** | ⚠️ ODC-BY + OpenAI ToU | ✅ deep + full scan |
| 2a | `allenai/WildChat-4.8M` | real-user logs | **3,199,860** | real prompts + model answers | gpt-3.5-turbo / gpt-4 | **8.32% = 266,258** | ⚠️ ODC-BY + OpenAI ToU | counts verified |
| 2b | **`lhoestq/Infinity-Instruct-Chinese-Only`** | mega-mixture (zh slice) | **751,313** | selection + evolution | mixed/unnamed | **100%** | ⚠️ mirror untagged; upstream CC-BY-SA-4.0 | ✅ deep |
| 2c | **`Magpie-Align/Magpie-Qwen2-Air-3M-v0.1`** | general chat pool | **3,000,000** | prompt-free self-synthesis | Qwen2-**7B**-Instruct | **71.1% = 2,133,622** | ⚠️ none on repo; **CC-BY-NC-4.0 per project** | ✅ 4-shard scan |
| 3 | `Magpie-Align/Magpie-Qwen2-Pro-200K-Chinese` | general chat | **200,000** | prompt-free self-synthesis | Qwen2-72B-Instruct | 100% | ⚠️ none on repo; **CC-BY-NC-4.0 per project** | ✅ deep |
| 3a | `Magpie-Align/Magpie-Qwen2-Pro-1M-v0.1` | general chat | **1,000,000** | prompt-free self-synthesis | Qwen2-72B-Instruct | **29.1% = 290,551** | ⚠️ same | counts verified |
...
| 27 | `allenai/tulu-3-sft-mixture` | English anchor | **939,343** | curated + persona synthesis | GPT-4o + Claude 3.5 Sonnet | **1.21% = 11,332** (from its WildChat + Aya subsamples) | ⚠️ ODC-BY + OpenAI/Anthropic ToU; NC subsets | ✅ full CJK scan |
| 28 | `Magpie-Align/Magpie-Qwen2.5-Pro-1M-v0.1` | general chat | **1,000,000** | prompt-free self-synthesis | Qwen2.5-72B-Instruct | **3.6% = 35,914** (0% in my 2 shards) | ⚠️ none on repo; CC-BY-NC-4.0 per project | ✅ 2-shard count |
...
| 32 | `BAAI/Infinity-Preference` | preference | **59,438** (train 59,338) | Gemma-2-9B-IT pairs + ArmoRM scores | Gemma-2-9B-IT | **8.4%** | ⚠️ apache tag **vs academic-only disclaimer** | ✅ (**mostly EN**) |
| 33 | `Skywork/Skywork-Reward-Preference-80K-v0.2` | preference | **77,016** | curated pairs | mixed | ~0% in sample | not stated | ✅ (**EN**) |
| 34 | `BAAI/Infinity-Instruct` | mega-mixture | **7,449,106** (`7M`) · 3M 3,463,473 · Gen 1,456,927 | selection + evolution | mixed | **10.09% (7M) / 21.62% (3M)** | ⚠️ CC-BY-SA-4.0 | ❌ gated — **use row 2b** |
| 35 | `lmsys/lmsys-chat-1m` | real-user logs | **1,000,000** | Chatbot Arena traffic | 25 LLMs | **2.45% = 24,499** (mirror-derived) | ⚠️ **no tag**; bespoke agreement | ❌ **gated** |
...
| 42 | `m-a-p/neo_sft_phase2` | general SFT | **109,447** (archived) | real user multi-turn dialogues | none named | **52.6% = 57,621** | — | ❌ **withdrawn; use mirrors** |
| 45 | Huozi 活字 `huozi_rlhf_data.csv` | preference | **16,918** | **human-annotated** over Huozi outputs | Huozi (own model) | 100% | ⚠️ Apache repo; commercial use needs written OK | ❌ GitHub-only |
| 46 | CValues-Comparison | preference | **145,669** | LLM generation + reward-model tiering | ChatGPT rewriting | 100% | ⚠️ mirrors untagged | ❌ not sampled |
| 47 | `ldwang/OpenHermes-2.5-zh` | translated mixture | **1,001,551** | 1:1 MT of OpenHermes-2.5 | translator **unnamed** | 100% (`zh_conversations`) | ⚠️ apache tag over an **unlicensed** source | ❌ not sampled |
...
**⚠️ No licence declared at all** (highest reuse risk) / **完全未声明许可（复用风险最高）:** `YeungNLP/firefly-train-1.1M`, `wangrui6/Zhihu-KOL` (scraped Zhihu content), `m-a-p/COIG-CQIA` ("[More Information Needed]"), `m-a-p/COIG-Kun` (dataset side), `FreedomIntelligence/Evol-Instruct-Chinese-GPT4`, `beyond/rlhf-reward-single-round-trans_chinese`, **every Qwen-generated Magpie repo**, `teknium/OpenHermes-2.5`, `lmsys/lmsys-chat-1m`, both `Skywork` Reward sets, `Skywork-OR1-RL-Data`, the a-m-team AM-Thinking / AM-Qwen3 / AM-DeepSeek-R1-0528 releases, `lhoestq/Infinity-Instruct-Chinese-Only` (mirror), the mxz / xiaodongguaAIGC CValues mirrors, `zake7749/kyara-*`, and **both Tulu `-0225` revisions**.

**⚠️ Self-contradicting licences** / **自相矛盾的许可:** `BAAI/Infinity-Preference` (`apache-2.0` tag vs. an academic-use-only disclaimer in its own card) · every BELLE repo (`gpl-3.0` tag vs. research-only prose) · MOSS (`cc-by-4.0` tags vs. CC-BY-NC in the project README) · `alpaca-gpt4-data-zh` (`cc-by-4.0` tag vs. CC-BY-NC-4.0 upstream) · `hfl/alpaca_zh_51k` (`apache-2.0` re-host vs. 严禁商用 source terms) · the Magpie Chinese repos (no tag vs. CC-BY-NC-4.0 in the project README) · `Mxode`/`ldwang` re-hosts asserting Apache-2.0 over sources that declare nothing.
...
**⚠️ Measured Chinese shares — the corrective table** / **实测中文占比——纠偏表.** Reputation is a poor guide here; these are counted, not assumed:

| Dataset | zh share | note |
|---|---|---|
| `Magpie-Qwen2-Air-3M-v0.1` | **71.1%** (2,133,622) | largest zh pool in this review, and its name says nothing about Chinese |
| `lhoestq/Infinity-Instruct-Chinese-Only` | **100%** (751,313) | reachable only via mirror |
| `m-a-p/neo_sft_phase2` | **52.6%** (57,621) | repo withdrawn |
| `Magpie-Qwen2-Pro-1M-v0.1` | 29.1% (290,551) | |
| `BAAI/Infinity-Instruct` `3M` | 21.62% (748,646) | gated |
| `allenai/WildChat-1M` | **14.24%** (119,302) | declines 25.5%→2.7% across time-ordered shards |
| `BAAI/Infinity-Instruct` `7M` | 10.09% (751,313) | gated |
| `allenai/WildChat-4.8M` | 8.32% (266,258) | |
| `BAAI/Infinity-Preference` | 8.4% | |
| `Magpie-Qwen2.5-Pro-1M-v0.1` | 3.6% (35,914) | 0% in the first 2 shards |
| `lmsys/lmsys-chat-1m` | 2.45% (24,499) | mirror-derived |
| `CohereLabs/aya_dataset` | 2.43% (4,909 train) | |
| `allenai/tulu-3-sft-mixture` | **1.21%** (11,332) | inherited from its WildChat + Aya subsamples |
| `Skywork-OR1-RL-Data` | 0.50% | RL prompts, not SFT |
| `teknium/OpenHermes-2.5` | 0.12% | |
| `Skywork-Reward-Preference-80K-v0.2` | **0.03%** | despite a Chinese lab and frequent miscitation |
| `Light-R1` SFTData | ~0% | 15 CJK characters in 71.8M |
...
**⚠️ Exact-duplicate and re-packaged repos** / **完全重复与再打包的库:** `shibing624/alpaca-zh` == `llm-wizard/alpaca-gpt4-data-zh` (same 48,818 rows; I verified offset-0 rows are byte-identical). `wenbopan/Chinese-dpo-pairs` == the zh half of `llamafactory/DPO-En-Zh-20k` == the zh half of `shibing624/DPO-En-Zh-20k-Preference`. `liyucheng/zhihu_rlhf_3k` == `ticoAg/zhihu_3k_rlhf_{train,test}`. CValues-Comparison has ≥5 HF mirrors. `Magpie-Qwen2-Pro-200K-Chinese` is a filtered subset of the Magpie Qwen2 pools. `Mxode/Chinese-Instruct`'s `infinity-instruct` config is a single-turn subset of `lhoestq/Infinity-Instruct-Chinese-Only`. And **Congliu-110k's largest prompt block (`coig/neo`, 52,893) is neo_sft_phase2's Chinese half re-answered by R1.** Counting these as independent sources inflates the apparent size of the open Chinese ecosystem substantially. | 上述各组为同一数据的重复或再打包，分别计数会显著夸大开源中文生态的表观规模。

**⚠️ Naming traps** / **命名陷阱:** Aya Collection's **`achinese` config (8,242,284 rows) is Acehnese (Indonesian), not Chinese** — a substring match on "chinese" pulls in 8.2M rows of the wrong language. `Skywork-OR1-RL-Data` is an **RL prompt set with no model responses**, not SFT data. `Magpie-Qwen2-Air-3M-v0.1` is the opposite trap: nothing in the name signals that it is ~71% Chinese.
...
44. Zhang et al. *MAP-Neo: Highly Capable and Transparent Bilingual Large Language Model Series* (source of `neo_sft_phase2`). arXiv:2403.19327. https://arxiv.org/abs/2403.19327
45. Liu et al. *Skywork-Reward: Bag of Tricks for Reward Modeling in LLMs*. arXiv:2410.18451. https://arxiv.org/abs/2410.18451
46. a-m-team. *AM-DeepSeek-R1-Distilled-1.4M: How Difficulty-Aware Data Selection Improves Reasoning Distillation*. arXiv:2503.19633. https://arxiv.org/abs/2503.19633
47. a-m-team. *AM-Thinking-v1: Advancing the Frontier of Reasoning at 32B Scale*. arXiv:2505.08311. https://arxiv.org/abs/2505.08311 · dataset report arXiv:2505.14464. https://arxiv.org/abs/2505.14464
48. Wen et al. *Light-R1: Curriculum SFT, DPO and RL for Long CoT from Scratch and Beyond*. arXiv:2503.10460. https://arxiv.org/abs/2503.10460
49. HIT-SCIR. *Huozi (活字) — an open Chinese conversational LLM*, incl. `huozi_rlhf_data.csv` (16,918 human-annotated preference pairs). https://github.com/HIT-SCIR/huozi
...
- **Corrections applied after a second verification pass** / **二次核验后的修正:** three figures in earlier drafts of this document were wrong and have been replaced by full-repo scans. (1) WildChat-1M Chinese: 23.27% (3 shards) → **14.24% (all 14 shards, 119,302 rows)**. (2) Tulu 3 Chinese: 2.15% (2 shards) → **1.21% (full repo, 11,332 rows)**. (3) `Magpie-Qwen2.5-Pro-1M-v0.1`: "zero Chinese" (2 shards) → **3.6% (35,914 rows)**. All three errors ran in the same direction and had the same cause — **HF Parquet shards are frequently ordered by collection time, so leading shards are not representative of language mix.** Any language share in this document derived from a partial scan now states the shard count explicitly. | 早期草稿中的三处数字有误，已以全库扫描替换（见上）。三处误差同向且同因：**HF Parquet 分片常按采集时间排序，靠前分片不能代表整体语言分布。** 本文凡由部分扫描得出的语言占比，均已明确标注所覆盖的分片数。
- **Access corrections** / **访问情况修正:** `BAAI/Infinity-Instruct` remains gated, but **the gate is not a dead end** — `lhoestq/Infinity-Instruct-Chinese-Only` (751,313 rows, 100% zh) and `manifoldlabs/Infinity-Instruct` (all configs) are ungated mirrors, and I sampled the former. Conversely, **`allenai/WildChat-1M` is NOT gated** (the AI2 ImpACT gate was lifted on 2024-06-26 with the move to ODC-BY); only the `-Full` toxic-inclusive variants require manual approval. | `BAAI/Infinity-Instruct` 仍受限，但**并非死路**：上述两个无门禁镜像可用，前者已实测。反之，**`allenai/WildChat-1M` 并未受限**，仅 `-Full` 变体需人工审批。
